                    ...
                    <div class="col-3  contact_category  text-center">
                        <div class="row">
                                <div class="contact_category_left col-12 col-lg-6">
                                    <a href="{{route('list_category-single-product',['slug'=>$product['slug']])}}">
                                    <img src="{{asset($product['image'])}}" class="sell_category_img">
                                    <span class="hidden-xs hidden-md" >{{$product['id']}}</span>
                                    </a>
                                </div>
                                <div class="contact_category_right hidden-xs hidden-md col-6 col-lg-6">
                                    <a href="{{route('list_category-single-product',['slug'=>$product['slug']])}}">
                                    <p class="title_cat1 light-black">{{$product['title']}}</p>
                                    <p class="title_cat2 light-black">{{$product['hr_tag']}}</p>
                                    </a>
                                </div>
                        </div>
                    </div>
                        ...
                        <div class="col-12">
                            <div class="widget widget-product card border_all  pad_time_prod" id="shegeft_1">
                                ...
                                <div class="product-carousel-catgory product-carousel owl-carousel owl-theme">
                                    @foreach($off_products as $item)
                                    <div class="item">
                                        <a href="{{route('list_category-single-product',['slug'=>$item['slug']])}}">
                                            <img src="{{asset($item['image'])}}" class="img-fluid" alt="">
                                        </a>
                                        <h2 class="product_title">
                                            <a href="{{route('list_category-single-product',['slug'=>$item['slug']])}}">{{$item['title']}}</a>
                                        </h2>
                                        <div class="price"><del><span>{{$item['old_price']}}<span>تومان</span></span></del>
                                            <span class="discount_badge">{{$item['discount']}}</span><ins><span>{{$item['new_price']}}<span>تومان</span></span></ins>
                                        </div>
                                    </div>
                                    @endforeach
                                </div>
                                <div class="product-carousel-catgory product-carousel owl-carousel owl-theme">
                                    @foreach($off_products2 as $item)
                                        <div class="item">
                                            <a href="{{route('list_category-single-product',['slug'=>$item['slug']])}}">
                                                <img src="{{asset($item['image'])}}" class="img-fluid" alt="">
                                            </a>
